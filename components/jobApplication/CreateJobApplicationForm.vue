<template>
  <Form
    :validation-schema="createJobApplicationSchema"
    @submit="handleCreateJobApplication"
  >
    ...
    <button v-else type="submit">Create Job Application</button>
  </Form>
</template>

<script setup lang="ts">
import { Form, Field, ErrorMessage } from 'vee-validate';
import createJobApplicationSchema from '@/models/jobApplication/validators/createJobApplicationSchema';
...
async function handleCreateJobApplication(values: any): Promise<void> {
  const { pending, error } = await useFetch('/api/job-applications', {
    method: 'POST',
    body: values,
  });
  ...
  if (pending.value) {
    ...
    formHandler.value.successMessage = 'Successfully created job application.';
  }
}
</script>
