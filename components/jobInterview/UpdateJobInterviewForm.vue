<template>
  <Form
    :validation-schema="updateJobInterviewSchema"
    @submit="handleUpdateJobInterview"
  >
    ...
    <button v-else type="submit">Update Job Interview</button>
  </Form>
</template>
...
async function handleUpdateJobInterview(values: any): Promise<void> {
  const { pending, error } = await useFetch(`/api/job-interviews/${id}`, {
    method: 'PUT',
    body: values,
  });
  ...
  if (pending.value) {
    ...
    formHandler.value.successMessage = 'Successfully updated job interview.';
  }
}
</script>
